...
const { default: makeWASocket, DisconnectReason, downloadContentFromMessage, useMultiFileAuthState, jidDecode, areJidsSameUser, makeInMemoryStore } = require('@adiwajshing/baileys')
const { state } = useMultiFileAuthState('./session')
const PhoneNumber = require('awesome-phonenumber')
const fs = require('fs')
const pino = require('pino')
const FileType = require('file-type')
const { Boom } = require('@hapi/boom')
const { smsg } = require('./module/myfunc')
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid } = require('./module/exif')
const chalk = require('chalk')
const color = (text, color) => { return !color ? chalk.green(text) : chalk.keyword(color)(text) }
const store = makeInMemoryStore({ logger: pino().child({ level: 'silent', stream: 'store' }) })

const connectToWhatsApp = () => {
const client = makeWASocket({ logger: pino ({ level: 'silent' }), printQRInTerminal: true, auth: state, browser: ["SC Simple By Ziro", "Dekstop", "3.0"]})
console.log(color('[ SC BY Ziro-MD ]\n', 'red')),
...
require('./casezaaxx')(client, msg, chatUpdate, store)
} catch (err) {
console.log(err)}})
...
const { getImg } = require('./module/functions')
